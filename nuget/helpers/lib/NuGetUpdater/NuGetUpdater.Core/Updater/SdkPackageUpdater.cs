...
internal static class SdkPackageUpdater
{
        ...
        ILogger logger)
    {
        ...
        {
            await UpdateDependencyWithConflictResolution(repoRootPath, buildFiles, tfms, projectPath, dependencyName, previousDependencyVersion, newDependencyVersion, isTransitive, logger);
        }
        else
        {
            if (isTransitive)
